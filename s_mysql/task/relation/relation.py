...
if __name__ == '__main__':
    ...
    # # #     ('또치', 9, '여자', parent_id)
    # # # )
    # # save_many_params = (('마이콜', 11, '선택안함', parent_id),)
    # #
    # # save_many(save_many_query, save_many_params)
    #
    # # 체험학습 추가
    ...
    #
    # # 체험학습 상세보기
    # find_by_id_query = "select id, title, content, count from tbl_field_trip where id = %s"
    # find_by_id_params = 4,
    ...
    # MySQL 암호화: hex(aes_encrypt(%s, '내가원하는키값'))으로 insert, decrypt로 select
    ...
    # 보호자 추가
    # save_query = "insert into tbl_owner(name, age, phone, address) \
    #               values(%s, %s, %s, %s)"
    # save_params = '한동석', 20, '01012341234', '경기도 남양주시'
    # save(save_query, save_params)

    # 반려동물 추가
    # find_by_id_query = "select id from tbl_owner where id = %s"
    # find_by_id_params = 1,
    # owner_id = find_by_id(find_by_id_query, find_by_id_params).get('id')
    #
    # save_query = "insert into tbl_pet(name, ill_name, age, weight, owner_id) \
    #               values (%s, %s, %s, %s, %s)"
    # # save_params = '누렁이', '감기', 5, 3.5, owner_id
    # save_params = '구름이', '타박상', 3, 4.5, owner_id
    # # save(save_query, save_params)
    #
    # # 전체 목록
    ...
    #     owners_with_pet.append(Owner(owner.get('id'), owner.get('name'), owner.get('age'), owner.get('phone'), owner.get('address'), pets))
    #
    # for owner in owners_with_pet:
    #     print(owner.__dict__)

    #############숙제##############
    # 회원가입
    # save_query = "insert into tbl_client (email, password, name) \
    #               values (%s, %s, %s)"
    # password = hashlib.sha256()
    # password.update('1234'.encode('utf-8'))
    # password = password.hexdigest()
    # save_params = 'hgd1234@gmail.com', password, '홍길동'
    # save(save_query, save_params)

    # 회사 추가
    # save_query = "insert into tbl_office (name, location) \
    #               values (%s, %s)"
    # save_params = "DS컨설팅", "서울시 강남구"
    # save_params = "한동석 법률사무소", "서울시 강남구"
    # save(save_query, save_params)
    ...
    # find_by_id_params = 1,
    # find_by_id_params = 2,
    # office_id = find_by_id(find_by_id_query, find_by_id_params).get('id')
    # save_query = "insert into tbl_conference_room (office_id) values (%s)"
    # save_params = office_id,
    # save(save_query, save_params)

    # 회의실마다 이용가능 시간 추가(파트타임)
    # find_by_id_query = "select id from tbl_conference_room where id = %s"
    # find_by_id_params = 5,
    # conference_room_id = find_by_id(find_by_id_query, find_by_id_params).get('id')
    # save_query = "insert into tbl_part_time (time, conference_room_id) \
    #               values(%s, %s)"
    # save_params = ('20:00', conference_room_id)
    # save(save_query, save_params)
    ...
    #     find_by_id_params = 'hgd1234@gmail.com',
